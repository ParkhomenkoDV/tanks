...
sys.path.append('D:\Programming\projects\games')

from pygame_tools import COLORS, draw_text

pygame.init()

WIDTH, HEIGHT = 800, 600
FPS = 60
TILE = 32  # размер квадратной картинки
    ...
    def draw(self):
        ...
        for obj in objects:
            if obj.type == 'tank':
                ...
                text = self.fonts[('None', 30)].render(str(obj.rank), 1, 'black')
                rect = text.get_rect(center=(5 + i * 70 + 11, 5 + 11))
                window.blit(text, rect)

                text = self.fonts[('None', 30)].render(str(obj.hp), 1, obj.color)
...
class Tank:
    imgTanks = [pygame.image.load(fr'images/tank{i + 1}.png') for i in range(8)]
    ...
    def __init__(self, color, px, py, direct, keyList):
        ...
        self.rank = 0
        self.image = pygame.transform.rotate(self.imgTanks[self.rank], -self.direct * 90)
...
class Bang:
    imgBangs = [pygame.image.load(fr'images/bang{i + 1}.png') for i in range(3)]
    ...
    def draw(self):
        image = self.imgBangs[int(self.frame)]
...
class Block:
    ...
    def draw(self):
        window.blit(self.imgBrick, self.rect)
...
class Bonus:
    imgBonuses = [pygame.image.load(fr'images/bonus_{i}.png') for i in ('star', 'tank')]

    def __init__(self, px, py, bonusNum):
        objects.append(self)
        self.type = 'bonus'

        self.image = self.imgBonuses[bonusNum]
    ...
    def update(self):
        ...
        for obj in objects:
            if obj.type == 'tank' and self.rect.colliderect(obj.rect):
                if self.bonusNum == 0:
                    if obj.rank < len(Tank.imgTanks) - 1:
                        obj.rank += 1
                        objects.remove(self)
                        break
